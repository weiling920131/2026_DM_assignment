...
print("\nRunning 5-fold CV on LightGBM-1 (quick check)...")
cv = cross_val_score(
    lgbm1, X, y,
    cv=StratifiedKFold(5, shuffle=True, random_state=SEED),
    scoring='f1_macro', n_jobs=-1
)
print(f"  LightGBM-1 CV F1 macro: {cv.mean():.4f} ± {cv.std():.4f}")
...
# LightGBM weighted higher (typically better on tabular data)
